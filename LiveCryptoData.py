...
class LiveCryptoData(object):
    ...
    def return_data(self):
        ...
        if self.verbose:
            tqdm.write("Collecting data for '{}'".format(self.ticker))

        self._ticker_checker()
        response = requests.get(
            "https://api.pro.coinbase.com/products/{}/ticker".format(self.ticker)
        )

        if response.status_code in [200, 201, 202, 203, 204]:
            if self.verbose:
                tqdm.write(
                    "Status Code: {}, successful API call.".format(response.status_code)
                )
