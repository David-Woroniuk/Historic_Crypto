...
class HistoricalData(object):
    ...
    def __init__(self, ticker, granularity, start_date, end_date=None, verbose=True):
        if verbose:
            tqdm.write("Checking input parameters are in the correct format.")
        if not all(isinstance(v, str) for v in [ticker, start_date]):
            raise TypeError(
                "The 'ticker' and 'start_date' arguments must be strings or None types."
            )
        ...
        if granularity not in [60, 300, 900, 3600, 21600, 86400]:
            raise ValueError(
                "'granularity' argument must be one of 60, 300, 900, 3600, 21600, 86400 seconds."
            )
        ...
        if not end_date:
            ...
            while start_date_datetime >= end_date_datetime:
                raise ValueError(
                    "'end_date' argument cannot occur prior to the start_date argument."
                )
    ...
    def _ticker_checker(self):
        ...
        if tkr_response.status_code in [200, 201, 202, 203, 204]:
            if self.verbose:
                tqdm.write("Connected to the CoinBase Pro API.")
            ...
            if self.verbose:
                tqdm.write(
                    "Status Code: {}, malformed request to the CoinBase Pro API.".format(
                        tkr_response.status_code
                    )
                )
            sys.exit()
            ...
            if self.verbose:
                tqdm.write(
                    "Status Code: {}, could not connect to the CoinBase Pro API.".format(
                        tkr_response.status_code
                    )
                )
            sys.exit()
            ...
            if self.verbose:
                tqdm.write(
                    "Status Code: {}, error in connecting to the CoinBase Pro API.".format(
                        tkr_response.status_code
                    )
                )
            sys.exit()
        ...
        if self.ticker in ticker_list:
            if self.verbose:
                tqdm.write(
                    "Ticker '{}' found at the CoinBase Pro API, continuing to extraction.".format(
                        self.ticker
                    )
                )
        else:
            raise ValueError(
                """Ticker: '{}' not available through CoinBase Pro API. Please use the Cryptocurrencies 
            class to identify the correct ticker.""".format(
                    self.ticker
                )
            )
    ...
    def retrieve_data(self):
        ...
        if request_volume <= 300:
            ...
            if response.status_code in [200, 201, 202, 203, 204]:
                if self.verbose:
                    tqdm.write("Retrieved Data from Coinbase Pro API.")
            ...
            for i in range(int(request_volume / max_per_mssg) + 1):
                provisional_start = start + timedelta(
                    0, i * (self.granularity * max_per_mssg)
                )
                provisional_start = self._date_cleaner(provisional_start)
                provisional_end = start + timedelta(
                    0, (i + 1) * (self.granularity * max_per_mssg)
                )
                provisional_end = self._date_cleaner(provisional_end)

                tqdm.write("Provisional Start: {}".format(provisional_start))
                tqdm.write("Provisional End: {}".format(provisional_end))
                response = requests.get(
                    "https://api.pro.coinbase.com/products/{0}/candles?start={1}&end={2}&granularity={3}".format(
                        self.ticker,
                        provisional_start,
                ...
                if response.status_code in [200, 201, 202, 203, 204]:
                    if self.verbose:
                        tqdm.write(
                            "Data for chunk {} of {} extracted".format(
                                i + 1, (int(request_volume / max_per_mssg) + 1)
                            )
                        )
                    ...
                    if not dataset.empty:
                        ...
                        tqdm.write(
                            """CoinBase Pro API did not have available data for '{}' beginning at {}.  
                        Trying a later date:'{}'""".format(
                                self.ticker, self.start_date, provisional_start
                            )
                        )
                        time.sleep(randint(0, 2))
                elif response.status_code in [400, 401, 404]:
