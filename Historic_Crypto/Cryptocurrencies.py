...
class Cryptocurrencies(object):
    ...
    def __init__(self, extended_output=False, coin_search=None, verbose=True):
        if not all(
            isinstance(v, (bool, type(None))) for v in [extended_output, verbose]
        ):
            raise TypeError(
                "The 'extended_output' and 'verbose' arguments must either be empty, or boolean types."
            )
        if not isinstance(coin_search, (str, type(None))):
            raise TypeError(
                "The 'coin_search' argument must either be empty, or a string type."
            )
    ...
    def find_crypto_pairs(self):
        ...
        if self.coin_search:
            ...
            if not outcome.empty:
                if self.verbose:
                    tqdm.write(
                        "Found {0} instances containing the term {1}.".format(
                            outcome.shape[0], self.coin_search
                        )
                    )
            else:
        ...
        if self.extended_output:
            if self.verbose:
                tqdm.write(
                    "Extended Output is {}, returning all data.".format(
            ...
            if self.verbose:
                tqdm.write(
                    "Extended Output is {}, returning reduced dataset.".format(
                        self.extended_output
                    )
                )
            outcome = outcome[
